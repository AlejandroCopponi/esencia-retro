...
import { Trash2, ArrowRight, Minus, Plus, CreditCard, ShieldCheck } from 'lucide-react';
import { useEffect, useState } from 'react';

export default function CarritoPage() {
  const { cart, removeFromCart, decreaseQuantity, increaseQuantity, total } = useCart();
  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center font-sans px-4 text-center" style={{ background: COLORS.gradient }}>
        ...
        <Link 
          href="/catalogo" 
          className="bg-[#0f0f10] text-white px-8 py-4 font-black uppercase tracking-widest hover:bg-[#c6a35a] hover:text-[#0f0f10] transition-colors shadow-lg"
        >
          Ir al Catálogo
        </Link>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-[#0f0f10]/10 sticky top-32">
              ...
              <button className="w-full bg-[#0f0f10] text-white py-4 font-black uppercase tracking-widest hover:bg-[#c6a35a] hover:text-[#0f0f10] transition-all transform hover:-translate-y-1 shadow-xl flex justify-center items-center gap-2 mb-4 rounded-sm">
